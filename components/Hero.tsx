  ...
  return (
    <div className="max-w-4xl w-full">
      <div className="flex flex-col space-y-6">
        <div className={`inline-block px-3 py-1 border mono text-[10px] sm:text-xs uppercase tracking-widest animate-pulse transition-all self-start ${
          isLightMode 
        ...
        <div className={`border p-5 sm:p-8 rounded-sm relative overflow-hidden backdrop-blur-sm min-h-[140px] transition-all shadow-sm ${
          ...
          <p className={`text-base sm:text-lg md:text-xl leading-relaxed relative z-10 transition-all mono ${isLightMode ? 'text-slate-600' : 'text-gray-400'}`}>
            {text}<span className={`inline-block w-2 h-5 ml-1 animate-pulse align-middle ${
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 sm:pt-8">
          {[
            ...
            <div key={idx} className={`border-l-2 pl-4 py-2 transition-all ${
              ...
              <div className={`mono text-base sm:text-lg font-bold transition-colors ${isLightMode ? 'text-slate-800' : 'text-gray-200'}`}>
                {stat.val}
              </div>
            </div>
          ))}
