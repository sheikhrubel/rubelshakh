...
const KnowledgeBase: React.FC<KnowledgeBaseProps> = ({ isHumanMode, isLightMode }) => {
    ...
    <div className="max-w-5xl">
      <div className="flex items-center space-x-4 mb-12">
        <h2 className={`mono text-2xl font-bold tracking-tighter transition-colors ${isLightMode ? 'text-slate-900' : 'text-white'}`}>
          {isHumanMode ? '05_GUIDES' : '05_KNOWLEDGE_BASE'}
        </h2>
      ...
      <div className={`border rounded overflow-hidden transition-all shadow-sm ${
        ...
        <div className={`border-b p-4 flex items-center justify-between transition-all ${
          isLightMode 
            ...
            : (isHumanMode ? 'bg-blue-950/20 border-blue-900/20' : 'bg-emerald-950/20 border-emerald-900/20')
        }`}>
          <div className={`mono text-xs font-bold transition-colors ${
            ...
            {isHumanMode ? 'HELPFUL_RESOURCES' : 'DOCUMENTATION_REPOSITORY'}
          </div>
        ...
        <div className={`divide-y transition-all ${isLightMode ? 'divide-slate-100' : (isHumanMode ? 'divide-blue-900/10' : 'divide-emerald-900/10')}`}>
          {articles.map((art) => (
            <div key={art.id} className={`p-4 flex items-center justify-between cursor-pointer group transition-all ${
              isLightMode ? 'hover:bg-slate-50' : (isHumanMode ? 'hover:bg-blue-500/5' : 'hover:bg-emerald-500/5')
            }`}>
              <div className="flex items-center space-x-4">
                <span className={`mono text-[10px] text-gray-400 transition-colors ${
                  isLightMode ? 'group-hover:text-slate-900' : (isHumanMode ? 'group-hover:text-blue-500' : 'group-hover:text-emerald-500')
                }`}>{art.id}</span>
                ...
                {art.tags.map(t => (
                  <span key={t} className={`mono text-[9px] px-1.5 py-0.5 border rounded-sm transition-all ${
                    isLightMode 
                      ...
                      : (isHumanMode ? 'border-blue-900/30 text-blue-600' : 'border-emerald-900/30 text-emerald-600')
                  }`}>{t}</span>
                ))}
              </div>
            </div>
        ...
        <div className={`p-8 text-center transition-colors ${isLightMode ? 'bg-slate-50' : 'bg-black/40'}`}>
          <p className="mono text-xs text-gray-500 mb-4 uppercase tracking-widest">{isHumanMode ? 'My Process' : 'Documentation Philosophy'}</p>
          <blockquote className={`text-xl italic font-serif max-w-2xl mx-auto transition-colors ${
            ...
            {isHumanMode 
              ? "\"I believe that clear instructions save everyone time and frustration.\""
              : "\"A solved problem without documentation is a problem waiting to happen again.\""}
          </blockquote>
        </div>
      </div>
    </div>
